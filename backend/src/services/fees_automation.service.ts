...
class FeesAutomationService {
  ...
  private async generateTransportRouteFees(schoolId: string, now: Date) {
    ...
    for (const route of routes || []) {
      ...
      // Paginated — a route with >1000 students must all be charged, and the
      // dedup check must see ALL existing rows or it would double-bill.
      ...
      for (const fee of feesToCharge) {
        ...
        const existing = await fetchAllRows<any>(
          supabase
            .from('fee_payments')
            .select('student_id')
            .eq('transport_route_id', route.id)
  ...
  private async generateNonMonthlyFees(schoolId: string, now: Date) {
    ...
    for (const freq of frequencies) {
      ...
      for (const structure of structures) {
        // Paginated — route/class/all student fetches must not truncate at 1000
        ...
        // Check existing (paginated — dedup must see ALL rows or it double-bills)
        const existing = await fetchAllRows<any>(
          supabase
            .from('fee_payments')
            .select('student_id')
  ...
  async runScheduledFeeReminders() {
    ...
    for (const school of schools || []) {
      // Paginated — a school with >1000 pending/overdue fees must still get
      // reminders for every one of them.
      const pendingFees = await fetchAllRows<any>(
        supabase
          .from('fee_payments')
          .select(`
      ...
      // Chunked — >1000 student ids overflows the PostgREST URL limit
      const parentLinkResults = await Promise.all(
      ...
      const allParentLinks = parentLinkResults.flatMap(r => r.data || []);
  ...
  async sendMonthlyProgressReports() {
    ...
    // Paginated — with >1000 active students, only the first 1000 would
    // otherwise receive the monthly progress report.
    const students = await fetchAllRows<any>(
      supabase
        .from('students')
        .select(`
  ...
  async sendAdminFeeDigest() {
    try {
      ...
      for (const school of schools || []) {
        ...
        // Get pending + overdue fees grouped by class/section (paginated —
        // counts/amounts would be wrong past 1000 rows)
        const pendingFees = await fetchAllRows<any>(
          supabase
            .from('fee_payments')
            .select(`
        ...
        // Get payments collected today (paginated)
        const todayPayments = await fetchAllRows<any>(
          supabase
            .from('fee_payments')
