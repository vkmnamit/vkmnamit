...
export const bulkIssueItem = async (req: Request, res: Response) => {
  ...
  // 1. Fetch Students — chunked because `student_ids` can exceed 1000 when a
  //    whole section/class is selected, which overflows the PostgREST URL limit.
  const studentFetchResults = await Promise.all(
    chunkArray(student_ids, 200).map(chunk =>
      supabase
        .from('students')
        .select('id, user_id')
  ...
  const studentFetchError = studentFetchResults.find(r => r.error)?.error;
  const students = studentFetchResults.flatMap(r => r.data || []);

  if (studentFetchError || !students || students.length === 0) {
    return res.status(400).json({ error: studentFetchError?.message || 'No valid students found for this issue' });
  }
  ...
  // Pre-fetch existing distributions if we don't allow duplicates (chunked —
  // `student_ids` can exceed 1000 and overflow the request URL)
  ...
  if (!allow_duplicate) {
    ...
    existingDistributions = distResults.flatMap(r => r.data || []);
  }
