...
async function purgeStudents(studentIds: string[]) {
  ...
  // Chunk every `.in()` — PostgREST caps un-paged queries at 1000 rows and
  // very long `.in()` lists overflow the request URL. Deleting a class/section
  // with >1000 students must not leave orphaned users behind.
  const idChunks = chunkArray(studentIds, 200);

  // 1. Get user_ids and auth_ids for all students
  const studentChunks = await Promise.all(
    idChunks.map(chunk =>
      supabaseAdmin.from('students').select('id, user_id').in('id', chunk)
    )
  );
  const students = studentChunks.flatMap(r => r.data || []);
  if (!students.length) return;
  const userIds = students.map(s => s.user_id).filter(Boolean);

  // 2. Get auth_ids from users table
  const userChunks = await Promise.all(
    chunkArray(userIds, 200).map(chunk =>
      supabaseAdmin.from('users').select('id, auth_id').in('id', chunk)
    )
  );
  const userRecords = userChunks.flatMap(r => r.data || []);
  ...
  // 4-6. Delete links, student records and user records (chunked)
  for (const chunk of idChunks) {
    await supabaseAdmin.from('parent_students').delete().in('student_id', chunk);
    await supabaseAdmin.from('students').delete().in('id', chunk);
  }
  for (const chunk of chunkArray(userIds, 200)) {
    await supabaseAdmin.from('users').delete().in('id', chunk);
  }
}
...
export async function deleteSection(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    // 1. Get all students in this section (paginated — a section can hold
    //    >1000 students and un-paged queries silently truncate to 1000)
    const students = await fetchAllRows<any>(
      supabaseAdmin.from('students').select('id').eq('section_id', sectionId)
    );
...
export async function deleteClass(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (sections && sections.length > 0) {
      ...
      // 2. Get all students across all sections (paginated — must not stop
      //    at 1000 rows or deleting a large class leaves orphans behind)
      const students = await fetchAllRows<any>(
        supabaseAdmin.from('students').select('id').in('section_id', sectionIds)
      );
...
export async function bulkPromoteStudents(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    // Chunked — `.in()` with >1000 ids overflows the PostgREST URL limit.
    const fetchResults = await Promise.all(
      chunkArray(studentIds, 200).map(chunk =>
        supabaseAdmin
          .from('students')
    ...
    if (fetchResults.some(r => r.error)) {
      return res.status(400).json({ error: fetchResults.find(r => r.error)!.error!.message });
    }
    const existing = fetchResults.flatMap(r => r.data || []);
    const validIds = existing.map((s: any) => s.id);
    if (validIds.length === 0) return res.status(404).json({ error: 'No matching students found' });

    // Batch update (chunked for the same URL-limit reason)
    ...
    try {
      ...
      // Chunked + paginated — validIds can exceed 1000 after a whole-school
      // promotion, and pending fees can exceed 1000 rows too.
      const feeFetchResults = await Promise.all(
        chunkArray(validIds, 200).map(chunk =>
          supabaseAdmin
            .from('fee_payments')
            .select('id')
            .eq('school_id', schoolId)
      ...
      const fetchErrChunk = feeFetchResults.find(r => r.error);
      const oldFees = feeFetchResults.flatMap(r => r.data || []);

      if (fetchErrChunk) {
        console.error('[PROMOTION] Error fetching old fees:', fetchErrChunk.error);
      } else if (oldFees.length > 0) {
        const oldFeeIds = oldFees.map(f => f.id);
        let updateFeesErr: any = null;
        for (const chunk of chunkArray(oldFeeIds, 200)) {
          const { error } = await supabaseAdmin
            .from('fee_payments')
            .update({ academic_year_id: targetAcademicYearId })
            .in('id', chunk);
          if (error) { updateFeesErr = error; break; }
        }
...
export async function generateFeesForExistingStudents(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    // Paginated fetch — a school with >1000 students must have fees generated
    // for ALL of them, not just the first 1000 (PostgREST default row cap).
    ...
    //    Paginated — >1000 rows would otherwise be truncated and cause duplicates.
