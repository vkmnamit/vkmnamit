...
export async function getExamTypes(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (!data || data.length === 0) {
      const defaults = [
        { school_id: schoolId, name: 'Mid-Term', weightage: 30, is_aggregate: true },
        { school_id: schoolId, name: 'Final Exam', weightage: 50, is_aggregate: true },
        { school_id: schoolId, name: 'Unit Test', weightage: 10, is_aggregate: false },
        { school_id: schoolId, name: 'Mock Test', weightage: 20, is_aggregate: false },
      ];
      let { data: seeded, error: seedErr } = await supabaseAdmin
        .from('exam_types')
        .insert(defaults)
        .select();
      if (seedErr && /is_aggregate|column/i.test(seedErr.message || '')) {
        // Migration not applied yet on this deployment — retry without the flag
        const retry = await supabaseAdmin
          .from('exam_types')
          .insert(defaults.map(({ is_aggregate, ...rest }) => rest))
          .select();
        seeded = retry.data;
        seedErr = retry.error;
      }
      ...
      return res.json({
        ...
        results: pastFiltered.slice(0, 10).map(r => {
          ...
          return {
            ...
            exam_type_id: r.exam_type_id || null,
...
export async function submitResults(req: AuthenticatedRequest, res: Response) {
    ...
    // NOTE: Marks can be entered at any time (draft or published). Visibility to
    // students/parents is gated by PUBLISHING (exam status = 'completed'), not by
    // the exam date — so teachers can pre-enter or correct marks freely.
    ...
    // Bulk upsert first (fast path — requires a UNIQUE(exam_id, student_id)
    // constraint on exam_results). If the live DB is missing that constraint the
    // whole upsert fails, so fall back to row-by-row update/insert so marks are
    // ALWAYS saved instead of silently reverting to defaults in the UI.
    ...
    if (error) {
      console.error('Bulk marks upsert failed, falling back to row-by-row save:', error.message);
      data = [];
      for (const rec of records) {
        ...
        if (existing) {
          const { data: updated, error: updErr } = await supabaseAdmin
            .from('exam_results')
            .update({ marks_obtained: rec.marks_obtained, grade: rec.grade, school_id: rec.school_id })
            .eq('id', existing.id)
            .select()
            .single();
...
export async function getReportCard(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (!isStaffViewer) {
      // PostgREST can't filter on nested join fields — pre-fetch published exam IDs
      const { data: publishedExams } = await supabaseAdmin
        .from('exams')
        .select('id')
        .eq('status', 'completed');
      const publishedIds = (publishedExams || []).map(e => e.id);
      if (publishedIds.length === 0) {
        return res.json({ student: null, subjects: [], overall: { percentage: 0, grade: 'N/A' }, results: [] });
      }
      query = query.in('exam_id', publishedIds);
    }
...
// Unpublish results — flips the exam back to draft ('scheduled') so marks can
// be edited again and are hidden from students/parents until re-published.
export async function unpublishResults(req: AuthenticatedRequest, res: Response) {
  try {
    const { examId } = req.body;
    if (!examId) return res.status(400).json({ error: 'examId is required' });

    // Same permission model as publishResults: teachers may only unpublish
    // exams they created or that belong to their assigned classes/sections.
    if (req.user!.role === 'teacher') {
      const { data: exam } = await supabaseAdmin
        .from('exams')
        .select('created_by, class_id, section_id')
      ...
      if (!isCreator && !isClassTeacher && !isSectionTeacher) {
        return res.status(403).json({ error: 'You do not have permission to unpublish results for this exam' });
      }
    }
    ...
    if (error) return res.status(400).json({ error: error.message });
    return res.json({ message: 'Results unpublished. Marks are editable again and hidden from students/parents.' });
  } catch (error: any) {
...
export async function getStudentAnalytics(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    let myResultsQuery = supabaseAdmin
      ...
      .eq('school_id', schoolId);
    if (!isStaffViewer) {
      // Pre-fetch published exam IDs (PostgREST can't filter on nested joins)
      const { data: pubExams } = await supabaseAdmin.from('exams').select('id').eq('status', 'completed').eq('school_id', schoolId);
      const pubIds = (pubExams || []).map(e => e.id);
      if (pubIds.length > 0) myResultsQuery = myResultsQuery.in('exam_id', pubIds);
    }
    ...
    if (!isStaffViewer) {
      // Already filtered via exam_ids above which came from published exams only
    }
...
export async function getClassReports(req: AuthenticatedRequest, res: Response) {
  try {
    const { class_id, section_id, exam_type_id } = req.query;
    if (!class_id || !exam_type_id) {
      return res.status(400).json({ error: 'class_id and exam_type_id are required' });
    }

    const scope = await getUserScope(req.user as any);
    if (req.user!.role === 'teacher' && scope) {
      if (section_id && !scope.sectionIds.includes(section_id as string)) {
        return res.status(403).json({ error: 'Unauthorized access to section' });
      } else if (!section_id && !scope.classIds.includes(class_id as string)) {
    ...
    let studentsQuery = supabaseAdmin
      .from('students')
      .select('id, roll_number, admission_number, section:sections(name), user:users(first_name, last_name)')
      .eq('class_id', class_id);
    if (section_id) studentsQuery = studentsQuery.eq('section_id', section_id);
    
    const { data: students } = await studentsQuery;

    let examsQuery = supabaseAdmin
      .from('exams')
      .select('id, total_marks, subject:subjects(name)')
      .eq('school_id', req.user!.school_id)
      .eq('class_id', class_id)
      .eq('exam_type_id', exam_type_id);
    // Include both section-specific and whole-class (null) exams
    if (section_id) {
      examsQuery = (examsQuery as any).or(`section_id.is.null,section_id.eq.${section_id}`);
    }

    const { data: exams } = await examsQuery;

    const examIds = (exams || []).map(e => e.id);
    const totalMax = (exams || []).reduce((sum, e) => sum + Number(e.total_marks || 0), 0);

    if (examIds.length === 0) {
      return res.json({ students: [], totalMax: 0 });
    }
    ...
    const studentTotals = new Map<string, { obtained: number }>();
    (students || []).forEach(s => {
      studentTotals.set(s.id, { obtained: 0 });
    });

    (results || []).forEach(r => {
      if (studentTotals.has(r.student_id)) {
        const m = r.marks_obtained === -1 || r.marks_obtained === null ? 0 : Number(r.marks_obtained);
        studentTotals.get(r.student_id)!.obtained += m;
      }
    });
    ...
    const studentData = (students || []).map(s => {
      const st = studentTotals.get(s.id)!;
      const pct = totalMax > 0 ? Math.round((st.obtained / totalMax) * 1000) / 10 : 0;
      return {
        id: s.id,
        name: `${(s as any).user?.first_name || ''} ${(s as any).user?.last_name || ''}`.trim(),
        rollNo: s.roll_number,
        admissionNo: s.admission_number,
        section: (s as any).section?.name || 'N/A',
        marksObtained: st.obtained,
        percentage: pct,
        grade: examGradeFor(pct)
      };
    });

    studentData.sort((a, b) => b.marksObtained - a.marksObtained);
    let currentRank = 0;
    let prevMarks: number | null = null;
    studentData.forEach((s, idx) => {
      if (s.marksObtained !== prevMarks) currentRank = idx + 1;
      (s as any).rank = currentRank;
      prevMarks = s.marksObtained;
    });

    return res.json({ students: studentData, totalMax, exams });
  } catch (err) {
    return res.status(500).json({ error: 'Failed to generate class reports' });
  }
}
