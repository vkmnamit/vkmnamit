...
/**
 * Batch-fetch parent user IDs for many students with ONE query (IN clause).
...
export async function getMonthlyGenerationStatus(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const [{ data: monthlyStructures, error: structuresError }, { data: payments, error: paymentsError }] = await Promise.all([
      supabaseAdmin
        .from('fee_structures')
        .select('id')
        .eq('school_id', schoolId)
    ...
    if (academic_year_id && academic_year_id !== 'all' && !class_id && !section_id && !student_id && !search) {
      ...
      if (yearStudentIds.length <= CHUNK) {
        ...
        for (const result of chunkResults) {
          if (result.data) allRecords = allRecords.concat(result.data);
        }
...
export async function collectFee(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const { data: receiptData } = await supabaseAdmin
      .rpc('generate_receipt_number', { p_school_id: req.user!.school_id });
...
export async function getFeeStats(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    // Limit to 50000 to ensure we get a full view of the school without unbounded memory
    ...
    if (!payments || payments.length === 0) return res.json({ total: 0, collected: 0, pending: 0, rate: 0, defaulters: 0, monthlyTrends: [] });
...
export async function bulkAssignFee(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (targetType === 'student') {
      ...
      const { data } = await supabaseAdmin.from('students').select('id').eq('section_id', targetId).eq('school_id', schoolId);
      studentIds = data?.map(s => s.id) || [];
    } else if (targetType === 'class') {
      const { data: sections } = await supabaseAdmin.from('sections').select('id').eq('class_id', targetId);
      const sectionIds = sections?.map(s => s.id) || [];
      if (sectionIds.length > 0) {
        const { data } = await supabaseAdmin.from('students').select('id').in('section_id', sectionIds).eq('school_id', schoolId);
        studentIds = data?.map(s => s.id) || [];
      }
    } else if (targetType === 'all') {
      const { data } = await supabaseAdmin.from('students').select('id').eq('school_id', schoolId);
      studentIds = data?.map(s => s.id) || [];
    }
