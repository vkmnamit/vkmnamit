...
export async function getStudentExamReports(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    // ── Academic-year support (schema-tolerant) ──
    // Aggregate exam types group PER ACADEMIC YEAR: Mid-Term 2025-26 and
    // Mid-Term 2024-25 stay separate. Prefer the exams.academic_year_id column;
    // if this deployment's DB doesn't have it yet, derive the year from the
    // exam date (April–March academic year).
    const { error: ayProbeErr } = await supabaseAdmin.from('exams').select('academic_year_id').limit(1);
    const hasAcademicYearCol = !ayProbeErr;
    const examSelectCols = `id, name, status, total_marks, date, exam_type_id${hasAcademicYearCol ? ', academic_year_id' : ''}, subject:subjects(name)`;

    // Academic-year names for report labels (tolerate a missing table)
    const { data: academicYearRows } = await supabaseAdmin
      .from('academic_years')
      .select('id, name');
    const ayNameMap = new Map<string, string>((academicYearRows || []).map((a: any) => [a.id, String(a.name)]));

    // Derive "YYYY-YY" academic year from a date (AY starts in April, month idx 3)
    const ayFromDate = (date: any): string | null => {
      if (!date) return null;
    ...
    // Stable grouping key for the academic year of an exam
    const ayKeyOf = (exam: any): string => {
      if (exam.academic_year_id) return `id:${exam.academic_year_id}`;
      const d = ayFromDate(exam.date);
      return d ? `d:${d}` : 'none';
    };
    // Human-readable academic-year label for report names
    const ayLabelOf = (exam: any): string => {
      if (exam.academic_year_id) return ayNameMap.get(exam.academic_year_id) || ayFromDate(exam.date) || '';
      return ayFromDate(exam.date) || '';
    };
    // For students/parents: pre-fetch only published exam IDs so they don't see unpublished marks
    let allowedExamIds: string[] | null = null;
    if (!isStaff) {
      const { data: pubExams } = await supabaseAdmin
        .from('exams')
        .select('id')
        .eq('school_id', req.user!.school_id)
        .eq('status', 'completed');
      allowedExamIds = (pubExams || []).map(e => e.id);
    }

    // ── 1. All exam results for this student ──
    let resultsQuery = supabaseAdmin
      .from('exam_results')
      .select(`
        id, marks_obtained, grade, remarks, created_at,
        exam:exams!inner(${examSelectCols})
      `)
      .eq('student_id', student.id)
      .order('created_at', { ascending: true });

    // Apply published-only filter at the query level (not nested join which doesn't work in PostgREST)
    if (allowedExamIds !== null) {
      if (allowedExamIds.length === 0) {
        return res.json({ student: { id: student.id, name: `${(student as any).user?.first_name || ''} ${(student as any).user?.last_name || ''}`.trim(), class: null, section: null, rollNumber: student.roll_number, admissionNumber: student.admission_number }, attendance: { rate: null, present: 0, total: 0 }, exams: [], overall: { totalExams: 0, totalObtained: 0, totalMax: 0, avgPercentage: null, grade: null, bestExam: null } });
      }
      resultsQuery = resultsQuery.in('exam_id', allowedExamIds);
    }
    ...
    // Try to load the is_aggregate flag too; older deployments without the
    // migration fall back to name-only lookup (name-based aggregation below).
    let examTypesData: any[] = [];
    let examTypesErr: any = null;
    {
      const withFlag = await supabaseAdmin.from('exam_types').select('id, name, is_aggregate');
      if (withFlag.error) {
        const noFlag = await supabaseAdmin.from('exam_types').select('id, name');
        examTypesData = noFlag.data || [];
        examTypesErr = noFlag.error;
      } else {
        examTypesData = withFlag.data || [];
        examTypesErr = withFlag.error;
      }
    }
    if (examTypesErr) {
      console.error('Exam types lookup error:', examTypesErr.message);
    }
    const examTypes = examTypesData;
    const typeNameMap = new Map<string, string>(examTypes.map((t: any) => [t.id, t.name]));

    // Aggregate exam types (e.g. Mid-Term, Final): ALL subject exams of the type
    // combine into ONE report card. Driven by the is_aggregate flag; if the
    // migration isn't applied yet we fall back to matching known type names.
    const AGGREGATE_TYPE_NAME_RE = /mid.?term|final|half.?yearly|annual|terminal/i;
    const aggregateTypeIds = new Set<string>(
      examTypes
        .filter((t: any) => {
          if (typeof t.is_aggregate === 'boolean') return t.is_aggregate;
          return AGGREGATE_TYPE_NAME_RE.test(String(t.name || ''));
        })
        .map((t: any) => t.id),
    );

    // Include all results that have an exam record.
    // For students/parents: the pre-fetched published IDs already limited to published exams via exam_id filter.
    // For staff: show all entered marks even if not yet published.
    const graded = (results || []).filter((r: any) => r.exam != null);


    // ── Also fetch ALL exams for this student's class so ungraded ones show as Pending ──
    const classId = (student.section as any)?.class?.id;
    let allClassExamsQuery = supabaseAdmin
      .from('exams')
      .select(examSelectCols)
      .eq('school_id', req.user!.school_id);

    if (classId) {
      allClassExamsQuery = allClassExamsQuery.eq('class_id', classId);
    }
    // Within this class: either whole-class (section_id null) or this specific section
    if (student.section_id) {
      allClassExamsQuery = (allClassExamsQuery as any).or(`section_id.is.null,section_id.eq.${student.section_id}`);
    }

    const { data: allClassExams } = await allClassExamsQuery.order('date', { ascending: true });


    // Build a set of exam IDs already in graded results
    const gradedExamIds = new Set(graded.map((r: any) => r.exam?.id));

    // Add "pending" placeholder rows for exams not yet graded
    const pendingRows = (allClassExams || [])
      .filter((e: any) => !gradedExamIds.has(e.id))
      .map((e: any) => ({
        id: null,
        marks_obtained: null,
        grade: null,
    ...
    const allRows = [...graded, ...pendingRows];
    ...
    // ── 4. Group results into exam reports ──
    // Aggregate types (Mid-Term, Final, ...): ALL subject exams of the type are
    // combined into ONE report card (subjects as rows, total, class position).
    // Everything else (Unit Tests etc.): one report PER EXAM so each shows
    // individually in the student's results dropdown. ──
    const groupKey = (r: any) => {
      const exam: any = r.exam || {};
      const isAggregate =
        (exam.exam_type_id && aggregateTypeIds.has(exam.exam_type_id)) ||
        AGGREGATE_TYPE_NAME_RE.test(String(exam.name || ''));
      if (isAggregate) {
        // Aggregate PER ACADEMIC YEAR: Mid-Term 2025-26 ≠ Mid-Term 2024-25
        const typeKey = exam.exam_type_id
          ? `t:${exam.exam_type_id}`
          : `n:${String(exam.name || 'Exam').trim().toLowerCase()}`;
        return `${typeKey}@${ayKeyOf(exam)}`;
      }
      return `e:${exam.id}`;
    };
    ...
    for (const [key, rows] of groups) {
      ...
      // Per-exam groups use the exam's own name (e.g. "Unit Test 3 · maths").
      // Aggregate groups ("t:"/"n:" keys) are named by the exam TYPE only
      // (e.g. "Mid-Term · 2026-27") since they span all subjects.
      const firstExam: any = rows[0].exam || {};
      const firstTypeName = firstExam.exam_type_id
        ? typeNameMap.get(firstExam.exam_type_id)
        : null;
      const examName = key.startsWith('e:')
        ? ([firstExam.name || firstTypeName, firstExam.subject?.name].filter(Boolean).join(' · ') || 'Exam')
        : ([firstTypeName || firstExam.name, ayLabelOf(firstExam)].filter(Boolean).join(' · ') || 'Exam');
