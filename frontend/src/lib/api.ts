...
export const api = {
  ...
  getClassReports: (params: { class_id: string; section_id?: string; exam_type_id: string }) => {
    const q = '?' + new URLSearchParams(params as any).toString();
    return fetchJson(`${API_BASE_URL}/exams/class-reports${q}`);
  },
  publishResults: (payload: any) => fetchJson(`${API_BASE_URL}/exams/publish`, { method: 'POST', body: JSON.stringify(payload) }),
  unpublishResults: (payload: any) => fetchJson(`${API_BASE_URL}/exams/unpublish`, { method: 'POST', body: JSON.stringify(payload) }),
  ...
  updateExpenseBill: (id: string, payload: any) => fetchJson(`${API_BASE_URL}/finance/expenses/${id}`, { method: 'PATCH', body: JSON.stringify(payload) }),
