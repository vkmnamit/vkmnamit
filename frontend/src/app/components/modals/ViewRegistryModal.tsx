...
export function ViewRegistryModal({ isOpen, onClose, exam, schoolName }: ViewRegistryModalProps) {
  ...
  const handleDownloadPDF = () => {
    ...
    const students = results.map(r => {
      const percentage = (r.marks_obtained / exam.total_marks) * 100;
      return {
    ...
    generateExamRegistryPDF({
      schoolName,
      examName: exam.name || exam.subject?.name || 'Examination',
      className: exam.class?.name ? `${exam.class.name} ${exam.section?.name ? `- ${exam.section.name}` : ''}` : 'All Classes',
      students
    });
  };
            ...
            <table className="w-full text-left text-sm whitespace-nowrap bg-white">
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredResults.map((r) => {
                  const percentage = r.marks_obtained >= 0 ? ((r.marks_obtained / exam.total_marks) * 100) : null;
                  ...
                  return (
                    <tr key={r.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className={`px-6 py-4 text-right font-black ${r.marks_obtained >= 0 ? 'text-blue-600' : 'text-slate-400'}`}>
                        {r.marks_obtained >= 0 ? `${r.marks_obtained} / ${exam.total_marks}` : 'N/A'}
                      </td>
