...
export function MarksEntryModal({ isOpen, onClose, exam, onSuccess }: MarksEntryModalProps) {
  ...
  // Pass/fail is decided by PERCENTAGE of total marks (not raw marks), so a
  // 30-mark student on a 50-mark exam (60%) passes even though 30 < 33.
  const totalMarks = Number(exam?.totalMarks || exam?.total_marks) || 0;
  const passingPercent = Number(exam?.passingMarks || exam?.passing_marks) || 33; // threshold as %
  const passingRaw = totalMarks > 0 ? Math.round((passingPercent / 100) * totalMarks * 10) / 10 : 0;
  const isPassingMark = (obtained: string) => {
    if (obtained === '' || obtained === null || obtained === undefined) return false;
    const marks = parseFloat(obtained);
    if (isNaN(marks)) return false;
    if (totalMarks > 0) return (marks / totalMarks) * 100 >= passingPercent;
    return marks >= passingPercent;
  };
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right hidden sm:block">
              ...
              <p className="text-lg font-bold text-rose-600">
                {passingPercent}% {totalMarks > 0 ? `(${passingRaw}/${totalMarks})` : ''}
              </p>
            </div>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                {filteredStudents.map((student) => {
                  ...
                  const isPassing = isPassingMark(res.marksObtained);
