...
{/* Overall Performance + Exam tabs */}
      <Card className="border-none shadow-sm bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 text-white overflow-hidden">
        <CardContent className="p-6">
          ...
          {/* Exam selector — dropdown listing ALL exams (graded + pending), one report per exam */}
          <div className="mt-5">
            <p className="text-[10px] font-black uppercase tracking-widest text-indigo-300 mb-2 flex items-center gap-1">
              <Award className="w-3 h-3" /> All Exams ({exams.length}) — select one to view its report
            </p>
            <Select value={selectedExamId ?? ''} onValueChange={(v) => setSelectedExamId(v)}>
              <SelectTrigger className="w-full sm:w-96 bg-white/10 border-white/20 text-white [&>svg]:text-indigo-200">
              ...
              <SelectContent className="max-h-72">
                {exams.map((exam) => (
                  <SelectItem key={exam.examId} value={exam.examId}>
                    ...
                    {exam.totalMax > 0 ? ` · ${exam.percentage}%` : ' · Pending'}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
