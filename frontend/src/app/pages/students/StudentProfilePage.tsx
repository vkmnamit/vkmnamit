...
export function StudentProfilePage() {
  ...
  const fetchAcademicData = async (sectionId: string, classId: string) => {
    try {
      // Load ALL exams for this class (no limit), and all results for this student
      const [classExams, reportData] = await Promise.all([
        api.getExams({ class_id: classId, limit: 500 }).catch(() => []),
        api.getReportCard(id!).catch(() => ({ results: [] }))
      ]);

      // getExams returns an array directly (non-dashboard mode)
      const allExams = Array.isArray(classExams) ? classExams : (classExams?.exams || []);
      // Filter to this section (or whole-class exams)
      const validExams = allExams.filter((e: any) => !e.section_id || e.section_id === sectionId);

      // Flatten all results from report card into a lookup map
      const resultsMap = new Map<string, any>();
      (reportData?.results || []).forEach((r: any) => {
        if (r.exam_id) resultsMap.set(r.exam_id, r);
      });
      ...
      mergedExams.sort((a: any, b: any) => new Date(b.exam.date || 0).getTime() - new Date(a.exam.date || 0).getTime());
