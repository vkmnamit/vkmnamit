...
import { Loader2, Search, Save, Send, AlertTriangle, Lock } from 'lucide-react';
...
export function MarksManagementPage() {
  ...
  const isExamLocked = (() => {
    if (!selectedExam?.date || assessmentType !== 'exam') return false;
          ...
          {isExamLocked && selectedExam && (
            <div className="mt-4 flex items-center gap-3 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl px-4 py-3">
              ...
              <div>
                ...
                <p className="text-xs font-medium">This exam is scheduled for <span className="font-black">{new Date(selectedExam.date).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span> and hasn't happened yet. Marks can be entered only after the exam date passes.</p>
              </div>
            </div>
          )}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            This action cannot be undone. This will permanently publish the results to all students and parents. Once published, you cannot edit these marks.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                    ...
                    <TableRow key={studentIdFromLMS(student)} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <TableCell>
                        <Input
                          ...
                          disabled={marksData[studentIdFromLMS(student)]?.isAbsent || selectedExam?.status === 'completed' || isExamLocked}
                      ...
                      <TableCell className="text-right">
                        <Switch
                          ...
                          disabled={selectedExam?.status === 'completed' || isExamLocked}
                        />
                      </TableCell>
                    </TableRow>
                  ))}
