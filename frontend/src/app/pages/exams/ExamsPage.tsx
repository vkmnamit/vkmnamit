...
import { AggregatedRegistryModal } from '../../components/modals/AggregatedRegistryModal';
...
export function ExamsPage() {
  ...
  const [aggregatedExamType, setAggregatedExamType] = useState<{ id: string; name: string; classId?: string } | null>(null);
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-x-hidden pb-24">
      ...
      <ViewRegistryModal
        ...
        schoolName={user?.school_name || 'Kautix International School'}
      />
      <AggregatedRegistryModal
        isOpen={isAggregatedModalOpen}
        onClose={() => { setIsAggregatedModalOpen(false); setAggregatedExamType(null); }}
        examTypeName={aggregatedExamType?.name || ''}
        examTypeId={aggregatedExamType?.id || ''}
        defaultClassId={aggregatedExamType?.classId}
        schoolName={user?.school_name || 'Kautix International School'}
      />
      <MarksEntryModal
                ...
                <CardContent className="p-0">
                  <div className="flex flex-col sm:flex-row sm:items-center">
                    ...
                    <div className="p-6 flex items-center gap-10 bg-gray-50/20 sm:bg-white sm:min-w-[380px] justify-between sm:justify-end">
                      ...
                      {result.exam_type_id && (
                        <Button
                          className="h-9 px-5 rounded-xl font-bold text-xs bg-blue-600 hover:bg-blue-700 text-white hidden md:flex"
                          onClick={() => {
                            setAggregatedExamType({ id: result.exam_type_id, name: result.name || 'Exam', classId: result.class_id });
