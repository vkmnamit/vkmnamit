...
const LEDGER_EXPORT_COLS = [
  ...
  { key: 'tx_amount', label: 'Transaction Amount (₹)' },
  { key: 'fee_title', label: 'Fee Title/Reason' },
  { key: 'total_fee_amount', label: 'Total Fee Amount (₹)' },
  { key: 'paid_amount', label: 'Paid Amount (₹)' },
  { key: 'late_fee', label: 'Late Fee (₹)' },
  { key: 'discount', label: 'Discount (₹)' },
  { key: 'roll_no', label: 'Roll No' },
  { key: 'student_name', label: 'Student Name/Payee' },
...
export function FeesPage() {
  ...
  // Export Column Picker States
  const [exportColPickerOpen, setExportColPickerOpen] = useState(false);
  const [selectedExportCols, setSelectedExportCols] = useState<string[]>(LEDGER_EXPORT_COLS.map(c => c.key));
  ...
  // Create Bill State (Global Ledger)
  ...
  // (large schools >1000 students — backend pages this when limit is high)
  useEffect(() => {
    if (user?.role === 'admin') {
      api.getStudents({ limit: '9999', status: 'active' }).then((res: any) => {
  ...
  const handleExportCSV = () => {
    ...
    // Build CSV (Merged Single Row Per Student)
    const headers = [
      'Roll No', 'Student Name', 'Admission No', 'Class', 'Section',
      'Father Name', 'Mother Name', 'Phone', 'Address',
      'Fee Details', 'Total Fee Amount (₹)', 'Total Late Fee (₹)', 'Total Discount (₹)',
      'Grand Total Due (₹)', 'Total Paid (₹)', 'Total Balance (₹)', 'Overall Status'
    ];
    ...
    rows.forEach((group: any) => {
      ...
      if (group.items.length === 0) {
        ...
        group.items.forEach((p: any) => {
          ...
          if (p.status !== 'paid') allPaid = false;
          if (p.status !== 'pending') allPending = false;

          const title = p.title || 'Fee';
          feeDetailParts.push(`${title}: ₹${totalDue} (Paid: ₹${paid})`);
        });

        let overallStatus = 'Partially Paid';
        if (allPaid) overallStatus = 'Paid';
        else if (allPending) overallStatus = 'Pending';
        else if (totalPaid === 0) overallStatus = 'Pending';

        const feeDetailsStr = feeDetailParts.join(' | ');

        csvLines.push([
          esc(rollNo), esc(name), esc(admNo), esc(className), esc(sectionName),
          esc(fatherName), esc(motherName), esc(phone), esc(address),
          esc(feeDetailsStr), String(totalFeeAmt), String(totalLateFee), String(totalDiscount),
          String(grandTotalDue), String(totalPaid), String(totalBalance), esc(overallStatus)
        ].join(','));
      }
    });
  ...
  const handleExportGlobalLedgerCSV = () => {
    ...
    setExportColPickerOpen(true);
  };

  const downloadGlobalLedgerCSV = () => {
    if (!globalTransactions || globalTransactions.length === 0) {
      toast.error('No transactions to export');
      return;
    }
    ...
    const headers = LEDGER_EXPORT_COLS
      .filter(col => selectedExportCols.includes(col.key))
      .map(col => col.label);
    ...
    globalTransactions.forEach((txn: any) => {
      ...
      const name = txn.is_expense
        ? (p.student?.user?.first_name || 'Expense')
        : (s.user ? `${s.user.first_name || ''} ${s.user.last_name || ''}`.trim() : (s.admission_number || 'Unknown'));
      const className = s.section?.class?.name || '';
      const sectionName = s.section?.name || '';
      const rollNo = s.roll_number || '';
      const admNo = s.admission_number || '';
      ...
      LEDGER_EXPORT_COLS.forEach(col => {
        if (!selectedExportCols.includes(col.key)) return;

        switch (col.key) {
          case 'receipt_number': rowCells.push(esc(receiptNo)); break;
          case 'payment_date': rowCells.push(esc(paymentDate)); break;
          case 'payment_method': rowCells.push(esc(paymentMethod)); break;
          case 'tx_amount': rowCells.push(String(txn.is_expense ? -txAmount : txAmount)); break;
          case 'fee_title': rowCells.push(esc(feeTitle)); break;
          case 'total_fee_amount': rowCells.push(String(totalFeeAmt)); break;
          case 'paid_amount': rowCells.push(String(paidAmt)); break;
          case 'late_fee': rowCells.push(String(lateFee)); break;
          case 'discount': rowCells.push(String(discount)); break;
          case 'roll_no': rowCells.push(esc(rollNo)); break;
          case 'student_name': rowCells.push(esc(name)); break;
          case 'admission_no': rowCells.push(esc(admNo)); break;
          case 'class': rowCells.push(esc(className)); break;
          case 'section': rowCells.push(esc(sectionName)); break;
          case 'father_name': rowCells.push(esc(fatherName)); break;
          case 'mother_name': rowCells.push(esc(motherName)); break;
          case 'phone': rowCells.push(esc(phone)); break;
          case 'address': rowCells.push(esc(address)); break;
        }
      });
  ...
  const handleCreateGlobalBill = async () => {
    if (!createBillForm.payee.trim()) { toast.error('Please enter who the bill is to'); return; }
    if (!createBillForm.reason.trim()) { toast.error('Please enter the reason'); return; }
    if (!createBillForm.amount || Number(createBillForm.amount) <= 0) { toast.error('Please enter a valid amount'); return; }

    try {
      setCreatingBill(true);
      await api.createExpense({
        payee: createBillForm.payee.trim(),
        reason: createBillForm.reason.trim(),
        title: createBillForm.reason.trim(),
        amount: Number(createBillForm.amount),
        category: createBillForm.category,
        paymentMethod: createBillForm.paymentMethod,
        status: 'paid', // Mark paid immediately so it shows up in global ledger
        date: new Date().toISOString().split('T')[0],
      });
      toast.success('Bill created & added to Global Ledger');
      setCreateBillModalOpen(false);
      setCreateBillForm({ payee: '', reason: '', amount: '', category: 'other', paymentMethod: 'cash' });
      fetchData(); // Refresh to show the new bill in global ledger
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-x-hidden pb-24">
          ...
          <Card className="border-none shadow-sm bg-white overflow-hidden">
            <CardHeader className="py-6 px-8 border-b border-gray-50 flex flex-row items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button className="h-9 px-4 rounded-lg text-xs font-bold bg-blue-600 hover:bg-blue-700" onClick={() => setCreateBillModalOpen(true)}>
              ...
              <Table>
                ...
                <TableBody>
                  ...
                  {(globalTransactions || []).map((txn: any) => (
                    <TableRow key={txn.id} className={`hover:bg-gray-50/50 transition-colors ${txn.is_expense ? 'bg-rose-50/10' : ''}`}>
                      <TableCell className="py-4 px-8">
                      ...
                      <TableCell className="py-4 px-6">
                        <p className="text-sm font-bold">
                          {txn.fee_payment?.student?.user?.first_name} {txn.fee_payment?.student?.user?.last_name}
                        </p>
                        {txn.is_expense && <p className="text-[10px] text-gray-500 font-medium">{txn.fee_payment?.title}</p>}
                      </TableCell>
                      <TableCell className="py-4 px-6">
                        <Badge variant="secondary" className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${txn.is_expense ? 'bg-rose-100 text-rose-700' : ''}`}>
                          {txn.payment_method || 'CASH'}
                        </Badge>
                      </TableCell>
                      <TableCell className={`py-4 px-8 text-right font-black flex items-center justify-end gap-3 ${txn.is_expense ? 'text-rose-600' : 'text-emerald-600'}`}>
                        {txn.is_expense ? '-' : '+'}₹{txn.amount.toLocaleString()}
                        {!txn.is_expense ? (
      ...
      {/* ── Create Bill Modal (Global Ledger) ────────────────────── */}
      <Dialog open={createBillModalOpen} onOpenChange={setCreateBillModalOpen}>
        <DialogContent className="max-w-md rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-base font-bold">Create Expense Bill</DialogTitle>
            <p className="text-xs text-gray-400 font-medium">Record a school expense — it will appear instantly in the Global Ledger.</p>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label className="text-[10px] font-bold uppercase text-gray-400">To Whom (Payee) *</Label>
              <Input
                placeholder="e.g. Ramesh Shopkeeper, Driver Vijay"
                value={createBillForm.payee}
                onChange={e => setCreateBillForm(f => ({ ...f, payee: e.target.value }))}
            ...
            <div className="space-y-1.5">
              ...
              <Input
                placeholder="e.g. Petrol for school bus, Inventory purchase"
                value={createBillForm.reason}
                onChange={e => setCreateBillForm(f => ({ ...f, reason: e.target.value }))}
                className="h-10 rounded-xl"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e => setCreateBillForm(f => ({ ...f, amount: e.target.value }))}
                className="h-10 rounded-xl"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Select value={createBillForm.category} onValueChange={v => setCreateBillForm(f => ({ ...f, category: v }))}>
                  <SelectTrigger className="h-10 rounded-xl"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="inventory">Inventory</SelectItem>
                    <SelectItem value="transport">Transport / Petrol</SelectItem>
              ...
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold uppercase text-gray-400">Payment Method</Label>
                <Select value={createBillForm.paymentMethod} onValueChange={v => setCreateBillForm(f => ({ ...f, paymentMethod: v }))}>
                  <SelectTrigger className="h-10 rounded-xl"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="cash">Cash</SelectItem>
                    <SelectItem value="bank_transfer">Bank Transfer</SelectItem>
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setCreateBillModalOpen(false)} className="rounded-xl">Cancel</Button>
            <Button onClick={handleCreateGlobalBill} disabled={creatingBill} className="bg-blue-600 hover:bg-blue-700 rounded-xl">
              {creatingBill ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <FileText className="w-4 h-4 mr-2" />}
              Create & Record Bill
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog open={exportColPickerOpen} onOpenChange={setExportColPickerOpen}>
        <DialogContent className="max-w-sm rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-base font-bold">Choose Export Columns</DialogTitle>
            <p className="text-xs text-gray-400 font-medium">Select which columns to include in the CSV.</p>
          </DialogHeader>
          <div className="space-y-2 py-2 max-h-72 overflow-y-auto">
            {LEDGER_EXPORT_COLS.map(col => (
              <label key={col.key} className="flex items-center gap-3 cursor-pointer p-2 rounded-lg hover:bg-gray-50">
                <input
                  type="checkbox"
                  checked={selectedExportCols.includes(col.key)}
                  onChange={e => {
                    if (e.target.checked) setSelectedExportCols(prev => [...prev, col.key]);
                    else setSelectedExportCols(prev => prev.filter(k => k !== col.key));
                  }}
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setSelectedExportCols(LEDGER_EXPORT_COLS.map(c => c.key))} className="rounded-xl text-xs">Select All</Button>
            <Button onClick={() => { downloadGlobalLedgerCSV(); }} className="bg-blue-600 hover:bg-blue-700 rounded-xl text-xs">
              <Download className="w-3.5 h-3.5 mr-2" />
              Download CSV
            </Button>
          </DialogFooter>
